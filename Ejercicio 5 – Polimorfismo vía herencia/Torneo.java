...
public class Torneo {
    ...
    public void cargarCSV(String archivo) {
        try (BufferedReader reader = new BufferedReader(new FileReader(archivo))) {
            ...
            while ((linea = reader.readLine()) != null) {
                // Analizar la línea y crear un jugador a partir de los datos
                Jugador jugador = Jugador.fromCSV(linea); // Supongamos que tienes un método "fromCSV" en la clase Jugador
    ...
    public List<Libero> obtenerMejoresLiberos(int cantidad) {
        ...
        // Filtrar y obtener solo los jugadores de tipo Libero
        for (Jugador jugador : jugadores) {
        ...
        Collections.sort(listaLiberos, new Comparator<Libero>() {
            ...
            public int compare(Libero libero1, Libero libero2) {
                // Comparar por efectividad en orden descendente
                return Double.compare(libero2.calcularEfectividad(), libero1.calcularEfectividad());
            }
        });

        // Tomar los mejores 'cantidad' Liberos (pueden ser menos si hay menos disponibles)
    ...
    public void guardarCSV(String archivo) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(archivo))) {
            for (Jugador jugador : jugadores) {
                writer.write(jugador.toCSV()); // Supongamos que tienes un método "toCSV" en la clase Jugador
                writer.newLine();
            }
